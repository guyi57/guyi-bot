    ...
    QObject::connect(reply, &QNetworkReply::finished, [reply, netMgr, callback]() {
        ...
        if (reply->error() == QNetworkReply::NoError) {
            ...
            while (it.hasNext() && results.size() < 6) {
                ...
                QString title;
                QRegularExpression titleRegex("<h2[^>]*><a[^>]*>(.*?)</a></h2>", QRegularExpression::DotMatchesEverythingOption);
                auto tMatch = titleRegex.match(block);
                if (tMatch.hasMatch()) {
                    title = cleanHtml(tMatch.captured(1));
                }
                ...
                QString snippet;
                QRegularExpression snippetRegex("<div class=\"b_caption\"[^>]*>.*?<p[^>]*>(.*?)</p>", QRegularExpression::DotMatchesEverythingOption);
...
void WebSearchEngine::search(const QString &query, std::function<void(bool success, const QVector<WebSearchResult>& results, const QString &formattedMarkdown)> callback)
{
    ...
    std::cout << "[WebSearchEngine] 正在并发检索 Baidu 与 Bing: " << trimmedQ.toStdString() << std::endl;
    ...
    // 并发启动百度与必应搜索
    queryBaidu(trimmedQ, onEngineDone);
